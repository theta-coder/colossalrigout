...
import { Truck, RotateCcw, ShieldCheck, MapPin, Heart, Star, Sparkles, X, ChevronLeft, ChevronRight, Headphones, Gift } from 'lucide-react';
...
export default function Home() {
  ...
  React.useEffect(() => {
    fetch('/api/reviews/latest?limit=5')
      .then(res => res.json())
      .then(data => {
  ...
  const [trustBenefits, setTrustBenefits] = React.useState<any[]>([]);
  React.useEffect(() => {
    fetch('/api/trust-benefits')
      .then(response => response.json())
      .then(payload => setTrustBenefits(payload.success ? payload.data || [] : []))
      .catch(() => setTrustBenefits([]));
  }, []);
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      {/* DYNAMIC SERVICES / TRUST STRIP */}
      {trustBenefits.length > 0 && <div className="border-t border-neutral-200 bg-[#fbfbfa] py-6 sm:py-8">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-3 text-center md:text-left">
            {trustBenefits.map(benefit => {
              const Icon = benefit.icon === 'truck' ? Truck : benefit.icon === 'returns' ? RotateCcw : benefit.icon === 'store' ? MapPin : benefit.icon === 'support' ? Headphones : benefit.icon === 'gift' ? Gift : ShieldCheck;
              return <div key={benefit.id} className="flex flex-col sm:flex-row items-center gap-2.5 justify-center md:justify-start">
                <Icon className="w-5 h-5 text-neutral-800 shrink-0" aria-hidden="true" />
                <div>
                  <p className="font-display font-bold text-xs uppercase tracking-wider text-neutral-900">{benefit.title}</p>
