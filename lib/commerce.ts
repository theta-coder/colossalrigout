export type CommerceResource = 'colors' | 'sizes' | 'size-guides' | 'collections' | 'reviews' | 'inventory' | 'trust-benefits';
...
export interface TrustBenefitRecord { id: string; title: string; subtitle: string; icon: 'truck' | 'returns' | 'shield' | 'store' | 'support' | 'gift'; active: boolean; order: number; createdAt: string; updatedAt: string; }
