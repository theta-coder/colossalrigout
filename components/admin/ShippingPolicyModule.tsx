  ...
  return <div className="max-w-4xl mx-auto space-y-6">
    ...
    <form onSubmit={saveSettings} className="bg-white border rounded-xl p-6 space-y-4">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        <label className="text-xs space-y-1"><span className="font-semibold">Free shipping threshold (PKR)</span><input type="number" min="0" value={settings.freeShippingThreshold ?? 2500} onChange={event => setSettings({ ...settings, freeShippingThreshold: Number(event.target.value) })} className="w-full border rounded-lg px-3 py-2" /></label>
        <label className="text-xs space-y-1"><span className="font-semibold">Flat shipping rate (PKR)</span><input type="number" min="0" value={settings.flatRate ?? 250} onChange={event => setSettings({ ...settings, flatRate: Number(event.target.value) })} className="w-full border rounded-lg px-3 py-2" /></label>
        <label className="text-xs space-y-1"><span className="font-semibold">Minimum delivery days</span><input type="number" min="1" value={settings.deliveryMinBusinessDays ?? 3} onChange={event => setSettings({ ...settings, deliveryMinBusinessDays: Number(event.target.value) })} className="w-full border rounded-lg px-3 py-2" /></label>
        <label className="text-xs space-y-1"><span className="font-semibold">Maximum delivery days</span><input type="number" min="1" value={settings.deliveryMaxBusinessDays ?? 7} onChange={event => setSettings({ ...settings, deliveryMaxBusinessDays: Number(event.target.value) })} className="w-full border rounded-lg px-3 py-2" /></label>
      </div>
